...
class M_Ringkasan extends CI_Model{
    ...
    public function __construct(){
        parent::__construct();
        $this->load->model(array('M_Kontrak','M_Adendum','M_Pembayaran','M_PerAkhir','M_Laporan','M_Rekanan','M_Satker','M_Paket'));
    }
    ...
    function base(){
        ...
        $this->db->join($this->M_Pembayaran->table,$this->M_Pembayaran->table.'.id_kontrak = '.$this->M_Kontrak->table.'.id_kontrak','LEFT');
    ...
    public function kontrak(){
        $this->db->select('COALESCE(sum('.$this->M_Laporan->table.'.realisasi),0) as realisasi,COALESCE(sum('.$this->M_Laporan->table.'.rencana),0) as rencana,COUNT('.$this->M_Adendum->table.'.id_addm) as jml_addm,COUNT('.$this->M_Pembayaran->table.'.id_pemb) as jml_pemb,COUNT('.$this->M_PerAkhir->table.'.id_prakhir) as jml_prakhir');
